...
export const Scene6_BetterPlans: React.FC = () => {
  ...
  // Enhanced sparkle particles - doubled for more magic effect
  const sparkles = Array.from({ length: 16 }, (_, i) => {
    const angle = (i / 16) * Math.PI * 2;
    const distance = 180 + Math.sin(frame * 0.1 + i) * 25;
    ...
    return {
      ...
      size: 5 + (i % 4) * 3, // More varied sizes
    };
  });
  ...
  return (
    ...
    >
      {/* Sound Effect: Magical chime for transformation */}
      <Audio
        src={staticFile(AUDIO_PATHS.sfx.chime)}
        volume={SFX_TIMING.scene6.chime.volume}
        startFrom={SFX_TIMING.scene6.chime.startFrom}
      />
